...
class TestEligibilityCheckEntity:

    def test_should_create_instance(self):
        testsdk = KotaSDK.test(None, None)
        ent = testsdk.EligibilityCheck(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _eligibility_check_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "eligibility_check." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set KOTA_TEST_ELIGIBILITY_CHECK_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        eligibility_check_ref01_ent = client.EligibilityCheck(None)
...
def _eligibility_check_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/eligibility_check/EligibilityCheckTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "KOTA_TEST_ELIGIBILITY_CHECK_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("KOTA_TEST_ELIGIBILITY_CHECK_ENTID"))
