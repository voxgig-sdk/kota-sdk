...
class KotaTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            KotaTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = KotaTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = KotaTestRunner.getenv("KOTA_TEST_LIVE")
        override = KotaTestRunner.getenv("KOTA_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = KotaTestRunner.getenv(key)
        ...
        explain = KotaTestRunner.getenv("KOTA_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["KOTA_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if KotaTestRunner._test_control is not None:
            return KotaTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                KotaTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            KotaTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = KotaTestRunner.load_test_control()
...
def load_env_local():
    KotaTestRunner.load_env_local()


def env_override(m):
    return KotaTestRunner.env_override(m)


def entity_data(v):
    return KotaTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return KotaTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return KotaTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return KotaTestRunner.load_test_control()


def live_client_options():
    return KotaTestRunner.live_client_options()


def live_delay_ms():
    return KotaTestRunner.live_delay_ms()
