...
class TestPolicyAmendmentIntentDirect:

    def test_should_direct_list_policy_amendment_intent(self):
        setup = _policy_amendment_intent_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-policy_amendment_intent", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
        if setup["live"]:
            for _live_key in ["policy_amendment_intent01"]:
    ...
    def test_should_direct_load_policy_amendment_intent(self):
        setup = _policy_amendment_intent_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-policy_amendment_intent", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
...
def _policy_amendment_intent_direct_setup(mockres):
