...
class PolicyAmendmentIntentEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return PolicyAmendmentIntentEntity(self._client, opts)
    ...
    def data_get(self) -> PolicyAmendmentIntent:
    ...
    def match_get(self) -> PolicyAmendmentIntent:
    ...
    def load(self, reqmatch=None, ctrl=None) -> PolicyAmendmentIntent:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.PolicyAmendmentIntent().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[PolicyAmendmentIntent]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.PolicyAmendmentIntent().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
    ...
    def create(self, reqdata: PolicyAmendmentIntentCreateData, ctrl=None) -> PolicyAmendmentIntent:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
