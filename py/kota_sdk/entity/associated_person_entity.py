...
from kota_sdk.kota_types import (
    AssociatedPerson,
...
class AssociatedPersonEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return AssociatedPersonEntity(self._client, opts)
    ...
    def data_get(self) -> AssociatedPerson:
    ...
    def match_get(self) -> AssociatedPerson:
    ...
    def load(self, reqmatch=None, ctrl=None) -> AssociatedPerson:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.AssociatedPerson().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[AssociatedPerson]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.AssociatedPerson().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
    ...
    def create(self, reqdata: AssociatedPersonCreateData, ctrl=None) -> AssociatedPerson:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
    ...
    def update(self, reqdata: AssociatedPersonUpdateData, ctrl=None) -> AssociatedPerson:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
    ...
    def remove(self, reqmatch=None, ctrl=None) -> AssociatedPerson:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.AssociatedPerson().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
