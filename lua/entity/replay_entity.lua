...
local ReplayEntity = {}
ReplayEntity.__index = ReplayEntity


function ReplayEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ReplayEntity)
  self._name = "replay"
  self._client = client
...
function ReplayEntity:get_name()
  return self._name
end


function ReplayEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ReplayEntity.new(self._client, opts)
end
...
function ReplayEntity:mark_deleted()
  self._deleted = true
end


function ReplayEntity:deleted()
  return true == self._deleted
end


function ReplayEntity:data_set(args)
  if args ~= nil then
...
function ReplayEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ReplayEntity:match_set(args)
  if args ~= nil then
...
function ReplayEntity:match_get()
...
function ReplayEntity:stream(action, args, callopts)
...
---@param reqdata ReplayCreateData
---@param ctrl? table
---@return Replay
---@return string? err
function ReplayEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function ReplayEntity:_run_op(ctx, post_done)
  local utility = self._utility
