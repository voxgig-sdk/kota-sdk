-- Kota SDK WebhookEndpointResponsePagedList entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local WebhookEndpointResponsePagedListEntity = {}
WebhookEndpointResponsePagedListEntity.__index = WebhookEndpointResponsePagedListEntity


function WebhookEndpointResponsePagedListEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, WebhookEndpointResponsePagedListEntity)
  self._name = "webhook_endpoint_response_paged_list"
  self._client = client
...
function WebhookEndpointResponsePagedListEntity:get_name()
  return self._name
end


function WebhookEndpointResponsePagedListEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return WebhookEndpointResponsePagedListEntity.new(self._client, opts)
end
...
function WebhookEndpointResponsePagedListEntity:mark_deleted()
  self._deleted = true
end


function WebhookEndpointResponsePagedListEntity:deleted()
  return true == self._deleted
end


function WebhookEndpointResponsePagedListEntity:data_set(args)
  if args ~= nil then
...
function WebhookEndpointResponsePagedListEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function WebhookEndpointResponsePagedListEntity:match_set(args)
  if args ~= nil then
...
function WebhookEndpointResponsePagedListEntity:match_get()
...
function WebhookEndpointResponsePagedListEntity:stream(action, args, callopts)
...
---@param reqmatch WebhookEndpointResponsePagedListListMatch
---@param ctrl? table
---@return WebhookEndpointResponsePagedList[]
---@return string? err
function WebhookEndpointResponsePagedListEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function WebhookEndpointResponsePagedListEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return WebhookEndpointResponsePagedListEntity
