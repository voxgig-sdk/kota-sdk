...
local PolicyEntity = {}
PolicyEntity.__index = PolicyEntity


function PolicyEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PolicyEntity)
  self._name = "policy"
  self._client = client
...
function PolicyEntity:get_name()
  return self._name
end


function PolicyEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PolicyEntity.new(self._client, opts)
end
...
function PolicyEntity:mark_deleted()
  self._deleted = true
end


function PolicyEntity:deleted()
  return true == self._deleted
end


function PolicyEntity:data_set(args)
  if args ~= nil then
...
function PolicyEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PolicyEntity:match_set(args)
  if args ~= nil then
...
function PolicyEntity:match_get()
...
function PolicyEntity:stream(action, args, callopts)
...
---@param reqmatch PolicyLoadMatch
---@param ctrl? table
---@return Policy
---@return string? err
function PolicyEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch PolicyListMatch
---@param ctrl? table
---@return Policy[]
---@return string? err
function PolicyEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function PolicyEntity:_run_op(ctx, post_done)
  local utility = self._utility
