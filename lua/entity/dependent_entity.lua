...
local DependentEntity = {}
DependentEntity.__index = DependentEntity


function DependentEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DependentEntity)
...
function DependentEntity:get_name()
  return self._name
end


function DependentEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DependentEntity.new(self._client, opts)
end
...
function DependentEntity:mark_deleted()
  self._deleted = true
end


function DependentEntity:deleted()
  return true == self._deleted
end


function DependentEntity:data_set(args)
  if args ~= nil then
...
function DependentEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DependentEntity:match_set(args)
  if args ~= nil then
...
function DependentEntity:match_get()
...
function DependentEntity:stream(action, args, callopts)
...
---@param reqdata DependentCreateData
---@param ctrl? table
---@return Dependent
---@return string? err
function DependentEntity:create(reqdata, ctrl)
  local utility = self._utility
...
---@param reqmatch DependentRemoveMatch
---@param ctrl? table
---@return Dependent
---@return string? err
function DependentEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function DependentEntity:_run_op(ctx, post_done)
  local utility = self._utility
