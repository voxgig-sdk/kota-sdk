-- Kota SDK EmployeeHealthInsuranceOffer entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local EmployeeHealthInsuranceOfferEntity = {}
EmployeeHealthInsuranceOfferEntity.__index = EmployeeHealthInsuranceOfferEntity


function EmployeeHealthInsuranceOfferEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EmployeeHealthInsuranceOfferEntity)
  self._name = "employee_health_insurance_offer"
  self._client = client
...
function EmployeeHealthInsuranceOfferEntity:get_name()
  return self._name
end


function EmployeeHealthInsuranceOfferEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EmployeeHealthInsuranceOfferEntity.new(self._client, opts)
end
...
function EmployeeHealthInsuranceOfferEntity:mark_deleted()
  self._deleted = true
end


function EmployeeHealthInsuranceOfferEntity:deleted()
  return true == self._deleted
end


function EmployeeHealthInsuranceOfferEntity:data_set(args)
  if args ~= nil then
...
function EmployeeHealthInsuranceOfferEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EmployeeHealthInsuranceOfferEntity:match_set(args)
  if args ~= nil then
...
function EmployeeHealthInsuranceOfferEntity:match_get()
...
function EmployeeHealthInsuranceOfferEntity:stream(action, args, callopts)
...
---@param reqmatch EmployeeHealthInsuranceOfferLoadMatch
---@param ctrl? table
---@return EmployeeHealthInsuranceOffer
---@return string? err
function EmployeeHealthInsuranceOfferEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function EmployeeHealthInsuranceOfferEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return EmployeeHealthInsuranceOfferEntity
