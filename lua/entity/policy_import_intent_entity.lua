...
local PolicyImportIntentEntity = {}
PolicyImportIntentEntity.__index = PolicyImportIntentEntity


function PolicyImportIntentEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PolicyImportIntentEntity)
  self._name = "policy_import_intent"
  self._client = client
...
function PolicyImportIntentEntity:get_name()
  return self._name
end


function PolicyImportIntentEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PolicyImportIntentEntity.new(self._client, opts)
end
...
function PolicyImportIntentEntity:mark_deleted()
  self._deleted = true
end


function PolicyImportIntentEntity:deleted()
  return true == self._deleted
end


function PolicyImportIntentEntity:data_set(args)
  if args ~= nil then
...
function PolicyImportIntentEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PolicyImportIntentEntity:match_set(args)
  if args ~= nil then
...
function PolicyImportIntentEntity:match_get()
...
function PolicyImportIntentEntity:stream(action, args, callopts)
...
---@param reqmatch PolicyImportIntentLoadMatch
---@param ctrl? table
---@return PolicyImportIntent
---@return string? err
function PolicyImportIntentEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch PolicyImportIntentListMatch
---@param ctrl? table
---@return PolicyImportIntent[]
---@return string? err
function PolicyImportIntentEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata PolicyImportIntentCreateData
---@param ctrl? table
---@return PolicyImportIntent
---@return string? err
function PolicyImportIntentEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function PolicyImportIntentEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return PolicyImportIntentEntity
