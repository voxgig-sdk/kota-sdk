-- Kota SDK EmployerHealthInsuranceQuote entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local EmployerHealthInsuranceQuoteEntity = {}
EmployerHealthInsuranceQuoteEntity.__index = EmployerHealthInsuranceQuoteEntity


function EmployerHealthInsuranceQuoteEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EmployerHealthInsuranceQuoteEntity)
  self._name = "employer_health_insurance_quote"
  self._client = client
...
function EmployerHealthInsuranceQuoteEntity:get_name()
  return self._name
end


function EmployerHealthInsuranceQuoteEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EmployerHealthInsuranceQuoteEntity.new(self._client, opts)
end
...
function EmployerHealthInsuranceQuoteEntity:mark_deleted()
  self._deleted = true
end


function EmployerHealthInsuranceQuoteEntity:deleted()
  return true == self._deleted
end


function EmployerHealthInsuranceQuoteEntity:data_set(args)
  if args ~= nil then
...
function EmployerHealthInsuranceQuoteEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EmployerHealthInsuranceQuoteEntity:match_set(args)
  if args ~= nil then
...
function EmployerHealthInsuranceQuoteEntity:match_get()
...
function EmployerHealthInsuranceQuoteEntity:stream(action, args, callopts)
...
---@param reqmatch EmployerHealthInsuranceQuoteLoadMatch
---@param ctrl? table
---@return EmployerHealthInsuranceQuote
---@return string? err
function EmployerHealthInsuranceQuoteEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function EmployerHealthInsuranceQuoteEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return EmployerHealthInsuranceQuoteEntity
