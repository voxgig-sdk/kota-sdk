...
EmployeeEntity.__index = EmployeeEntity


function EmployeeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EmployeeEntity)
  self._name = "employee"
  self._client = client
...
function EmployeeEntity:get_name()
  return self._name
end


function EmployeeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EmployeeEntity.new(self._client, opts)
end
...
function EmployeeEntity:mark_deleted()
  self._deleted = true
end


function EmployeeEntity:deleted()
  return true == self._deleted
end


function EmployeeEntity:data_set(args)
  if args ~= nil then
...
function EmployeeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EmployeeEntity:match_set(args)
  if args ~= nil then
...
function EmployeeEntity:match_get()
...
function EmployeeEntity:stream(action, args, callopts)
...
---@param reqmatch EmployeeLoadMatch
---@param ctrl? table
---@return Employee
---@return string? err
function EmployeeEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch EmployeeListMatch
---@param ctrl? table
---@return Employee[]
---@return string? err
function EmployeeEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata EmployeeCreateData
---@param ctrl? table
---@return Employee
---@return string? err
function EmployeeEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata EmployeeUpdateData
---@param ctrl? table
---@return Employee
---@return string? err
function EmployeeEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function EmployeeEntity:_run_op(ctx, post_done)
  local utility = self._utility
