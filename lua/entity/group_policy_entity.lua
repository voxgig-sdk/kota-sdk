...
local GroupPolicyEntity = {}
GroupPolicyEntity.__index = GroupPolicyEntity


function GroupPolicyEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GroupPolicyEntity)
  self._name = "group_policy"
  self._client = client
...
function GroupPolicyEntity:get_name()
  return self._name
end


function GroupPolicyEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GroupPolicyEntity.new(self._client, opts)
end
...
function GroupPolicyEntity:mark_deleted()
  self._deleted = true
end


function GroupPolicyEntity:deleted()
  return true == self._deleted
end


function GroupPolicyEntity:data_set(args)
  if args ~= nil then
...
function GroupPolicyEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GroupPolicyEntity:match_set(args)
  if args ~= nil then
...
function GroupPolicyEntity:match_get()
...
function GroupPolicyEntity:stream(action, args, callopts)
...
---@param reqmatch GroupPolicyLoadMatch
---@param ctrl? table
---@return GroupPolicy
---@return string? err
function GroupPolicyEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch GroupPolicyListMatch
---@param ctrl? table
---@return GroupPolicy[]
---@return string? err
function GroupPolicyEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function GroupPolicyEntity:_run_op(ctx, post_done)
  local utility = self._utility
