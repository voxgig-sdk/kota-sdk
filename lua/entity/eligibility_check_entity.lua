...
local EligibilityCheckEntity = {}
EligibilityCheckEntity.__index = EligibilityCheckEntity


function EligibilityCheckEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EligibilityCheckEntity)
  self._name = "eligibility_check"
  self._client = client
...
function EligibilityCheckEntity:get_name()
  return self._name
end


function EligibilityCheckEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EligibilityCheckEntity.new(self._client, opts)
end
...
function EligibilityCheckEntity:mark_deleted()
  self._deleted = true
end


function EligibilityCheckEntity:deleted()
  return true == self._deleted
end


function EligibilityCheckEntity:data_set(args)
  if args ~= nil then
...
function EligibilityCheckEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EligibilityCheckEntity:match_set(args)
  if args ~= nil then
...
function EligibilityCheckEntity:match_get()
...
function EligibilityCheckEntity:stream(action, args, callopts)
...
---@param reqdata EligibilityCheckCreateData
---@param ctrl? table
---@return EligibilityCheck
---@return string? err
function EligibilityCheckEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function EligibilityCheckEntity:_run_op(ctx, post_done)
  local utility = self._utility
