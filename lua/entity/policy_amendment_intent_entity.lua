...
local PolicyAmendmentIntentEntity = {}
PolicyAmendmentIntentEntity.__index = PolicyAmendmentIntentEntity


function PolicyAmendmentIntentEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PolicyAmendmentIntentEntity)
  self._name = "policy_amendment_intent"
  self._client = client
...
function PolicyAmendmentIntentEntity:get_name()
  return self._name
end


function PolicyAmendmentIntentEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PolicyAmendmentIntentEntity.new(self._client, opts)
end
...
function PolicyAmendmentIntentEntity:mark_deleted()
  self._deleted = true
end


function PolicyAmendmentIntentEntity:deleted()
  return true == self._deleted
end


function PolicyAmendmentIntentEntity:data_set(args)
  if args ~= nil then
...
function PolicyAmendmentIntentEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PolicyAmendmentIntentEntity:match_set(args)
  if args ~= nil then
...
function PolicyAmendmentIntentEntity:match_get()
...
function PolicyAmendmentIntentEntity:stream(action, args, callopts)
...
---@param reqmatch PolicyAmendmentIntentLoadMatch
---@param ctrl? table
---@return PolicyAmendmentIntent
---@return string? err
function PolicyAmendmentIntentEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch PolicyAmendmentIntentListMatch
---@param ctrl? table
---@return PolicyAmendmentIntent[]
---@return string? err
function PolicyAmendmentIntentEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata PolicyAmendmentIntentCreateData
---@param ctrl? table
---@return PolicyAmendmentIntent
---@return string? err
function PolicyAmendmentIntentEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function PolicyAmendmentIntentEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return PolicyAmendmentIntentEntity
