...
local PlanEntity = {}
PlanEntity.__index = PlanEntity


function PlanEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PlanEntity)
  self._name = "plan"
  self._client = client
...
function PlanEntity:get_name()
  return self._name
end


function PlanEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PlanEntity.new(self._client, opts)
end
...
function PlanEntity:mark_deleted()
  self._deleted = true
end


function PlanEntity:deleted()
  return true == self._deleted
end


function PlanEntity:data_set(args)
  if args ~= nil then
...
function PlanEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PlanEntity:match_set(args)
  if args ~= nil then
...
function PlanEntity:stream(action, args, callopts)
...
---@param reqmatch PlanLoadMatch
---@param ctrl? table
---@return Plan
---@return string? err
function PlanEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch PlanListMatch
---@param ctrl? table
---@return Plan[]
---@return string? err
function PlanEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function PlanEntity:_run_op(ctx, post_done)
  local utility = self._utility
