...
local GroupQuoteEntity = {}
GroupQuoteEntity.__index = GroupQuoteEntity


function GroupQuoteEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GroupQuoteEntity)
  self._name = "group_quote"
  self._client = client
...
function GroupQuoteEntity:get_name()
  return self._name
end


function GroupQuoteEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GroupQuoteEntity.new(self._client, opts)
end
...
function GroupQuoteEntity:mark_deleted()
  self._deleted = true
end


function GroupQuoteEntity:deleted()
  return true == self._deleted
end


function GroupQuoteEntity:data_set(args)
  if args ~= nil then
...
function GroupQuoteEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GroupQuoteEntity:match_set(args)
  if args ~= nil then
...
function GroupQuoteEntity:match_get()
...
function GroupQuoteEntity:stream(action, args, callopts)
...
---@param reqmatch GroupQuoteLoadMatch
---@param ctrl? table
---@return GroupQuote
---@return string? err
function GroupQuoteEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function GroupQuoteEntity:_run_op(ctx, post_done)
  local utility = self._utility
