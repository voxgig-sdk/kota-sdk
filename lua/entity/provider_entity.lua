...
local ProviderEntity = {}
ProviderEntity.__index = ProviderEntity


function ProviderEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ProviderEntity)
  self._name = "provider"
  self._client = client
...
function ProviderEntity:get_name()
  return self._name
end


function ProviderEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ProviderEntity.new(self._client, opts)
end
...
function ProviderEntity:mark_deleted()
  self._deleted = true
end


function ProviderEntity:deleted()
  return true == self._deleted
end


function ProviderEntity:data_set(args)
  if args ~= nil then
...
function ProviderEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ProviderEntity:match_set(args)
  if args ~= nil then
...
function ProviderEntity:match_get()
...
function ProviderEntity:stream(action, args, callopts)
...
---@param reqmatch ProviderLoadMatch
---@param ctrl? table
---@return Provider
---@return string? err
function ProviderEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ProviderEntity:_run_op(ctx, post_done)
  local utility = self._utility
