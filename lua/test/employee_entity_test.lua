...
describe("EmployeeEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:Employee(nil)
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Employee(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Employee(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    local setup = employee_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create", "list", "update", "load"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "employee." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set KOTA_TEST_EMPLOYEE_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local employee_ref01_ent = client:Employee(nil)
    local employee_ref01_data = helpers.to_map(vs.getprop(
    ...
    -- LIST
    local employee_ref01_match = {}

    local employee_ref01_list_result, err = employee_ref01_ent:list(employee_ref01_match, nil)
    assert.is_nil(err)
    assert.is_table(employee_ref01_list_result)
    ...
    -- UPDATE
    local employee_ref01_data_up0_up = {
      id = employee_ref01_data["id"],
    }

    local employee_ref01_markdef_up0_name = "date_of_birth"
    local employee_ref01_markdef_up0_value = "Mark01-employee_ref01_" .. tostring(setup.now)
    ...
    -- LOAD
    local employee_ref01_match_dt0 = {
      id = employee_ref01_data["id"],
    }
...
function employee_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/employee/EmployeeTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read employee test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("KOTA_TEST_EMPLOYEE_ENTID")
