...
describe("PolicyAmendmentIntentDirect", function()
  it("should direct-list-policy_amendment_intent", function()
    local setup = policy_amendment_intent_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-policy_amendment_intent", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"policy_amendment_intent01"}) do
  ...
  it("should direct-load-policy_amendment_intent", function()
    local setup = policy_amendment_intent_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-policy_amendment_intent", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
