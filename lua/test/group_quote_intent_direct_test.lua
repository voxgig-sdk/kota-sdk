...
describe("GroupQuoteIntentDirect", function()
  it("should direct-list-group_quote_intent", function()
    local setup = group_quote_intent_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-group_quote_intent", setup.live and "live" or "unit")
  ...
  it("should direct-load-group_quote_intent", function()
    local setup = group_quote_intent_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-group_quote_intent", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function group_quote_intent_direct_setup(mockres)
