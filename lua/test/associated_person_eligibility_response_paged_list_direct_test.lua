...
describe("AssociatedPersonEligibilityResponsePagedListDirect", function()
  it("should direct-list-associated_person_eligibility_response_paged_list", function()
    local setup = associated_person_eligibility_response_paged_list_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-associated_person_eligibility_response_paged_list", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"dependents_management_intent01"}) do
...
function associated_person_eligibility_response_paged_list_direct_setup(mockres)
