...
describe("EligibilityCheckEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:EligibilityCheck(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = eligibility_check_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "eligibility_check." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set KOTA_TEST_ELIGIBILITY_CHECK_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local eligibility_check_ref01_ent = client:EligibilityCheck(nil)
    local eligibility_check_ref01_data = helpers.to_map(vs.getprop(
...
function eligibility_check_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/eligibility_check/EligibilityCheckTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read eligibility_check test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("KOTA_TEST_ELIGIBILITY_CHECK_ENTID")
