...
describe("EmployerDirect", function()
  it("should direct-list-employer", function()
    local setup = employer_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-employer", setup.live and "live" or "unit")
  ...
  it("should direct-load-employer", function()
    local setup = employer_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-employer", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function employer_direct_setup(mockres)
