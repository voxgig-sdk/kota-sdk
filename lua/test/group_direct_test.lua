...
describe("GroupDirect", function()
  it("should direct-list-group", function()
    local setup = group_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-group", setup.live and "live" or "unit")
  ...
  it("should direct-load-group", function()
    local setup = group_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-group", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    ...
    local query = {}
    if setup.live then
      params["id"] = "gr_3b1333d87d9d4fd6ad83ba7f6b0e951a"
    else
      params["id"] = "direct01"
    end
...
function group_direct_setup(mockres)
  runner.load_env_local()

  local calls = {}

  local env = runner.env_override({
    ["KOTA_TEST_GROUP_ENTID"] = {},
