...
describe("CreateHostedSessionTokenEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:CreateHostedSessionToken(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = create_hosted_session_token_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "create_hosted_session_token." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set KOTA_TEST_CREATE_HOSTED_SESSION_TOKEN_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local create_hosted_session_token_ref01_ent = client:CreateHostedSessionToken(nil)
    local create_hosted_session_token_ref01_data = helpers.to_map(vs.getprop(
...
function create_hosted_session_token_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/create_hosted_session_token/CreateHostedSessionTokenTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read create_hosted_session_token test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("KOTA_TEST_CREATE_HOSTED_SESSION_TOKEN_ENTID")
