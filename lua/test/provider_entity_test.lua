...
describe("ProviderEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Provider(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Provider(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set KOTA_TEST_PROVIDER_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local provider_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.provider")))
    local provider_ref01_data = nil
    if #provider_ref01_data_raw > 0 then
      provider_ref01_data = helpers.to_map(provider_ref01_data_raw[1][2])
    end

    -- LIST
    local provider_ref01_ent = client:Provider(nil)
    local provider_ref01_match = {}

    local provider_ref01_list_result, err = provider_ref01_ent:list(provider_ref01_match, nil)
    assert.is_nil(err)
    assert.is_table(provider_ref01_list_result)
...
function provider_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("KOTA_TEST_PROVIDER_ENTID")
