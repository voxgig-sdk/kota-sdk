...
describe("GroupPolicyIntentEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:GroupPolicyIntent(nil)
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:GroupPolicyIntent(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:GroupPolicyIntent(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    local setup = group_policy_intent_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create", "list", "load"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "group_policy_intent." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set KOTA_TEST_GROUP_POLICY_INTENT_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local group_policy_intent_ref01_ent = client:GroupPolicyIntent(nil)
    local group_policy_intent_ref01_data = helpers.to_map(vs.getprop(
    ...
    -- LIST
    local group_policy_intent_ref01_match = {}

    local group_policy_intent_ref01_list_result, err = group_policy_intent_ref01_ent:list(group_policy_intent_ref01_match, nil)
    assert.is_nil(err)
    assert.is_table(group_policy_intent_ref01_list_result)
    ...
    -- LOAD
    local group_policy_intent_ref01_match_dt0 = {
      id = group_policy_intent_ref01_data["id"],
    }
...
function group_policy_intent_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/group_policy_intent/GroupPolicyIntentTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read group_policy_intent test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("KOTA_TEST_GROUP_POLICY_INTENT_ENTID")
