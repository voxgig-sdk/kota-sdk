...
describe("EmployeeDirect", function()
  it("should direct-list-employee", function()
    local setup = employee_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-employee", setup.live and "live" or "unit")
  ...
  it("should direct-load-employee", function()
    local setup = employee_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-employee", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function employee_direct_setup(mockres)
