...
describe("PolicyDirect", function()
  it("should direct-list-policy", function()
    local setup = policy_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-policy", setup.live and "live" or "unit")
  ...
  it("should direct-load-policy", function()
    local setup = policy_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-policy", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function policy_direct_setup(mockres)
