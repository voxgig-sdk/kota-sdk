...
describe("PolicyImportIntentEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:PolicyImportIntent(nil)
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:PolicyImportIntent(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:PolicyImportIntent(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    local setup = policy_import_intent_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create", "list", "load"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "policy_import_intent." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set KOTA_TEST_POLICY_IMPORT_INTENT_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local policy_import_intent_ref01_ent = client:PolicyImportIntent(nil)
    local policy_import_intent_ref01_data = helpers.to_map(vs.getprop(
    ...
    -- LIST
    local policy_import_intent_ref01_match = {}

    local policy_import_intent_ref01_list_result, err = policy_import_intent_ref01_ent:list(policy_import_intent_ref01_match, nil)
    assert.is_nil(err)
    assert.is_table(policy_import_intent_ref01_list_result)
    ...
    -- LOAD
    local policy_import_intent_ref01_match_dt0 = {
      id = policy_import_intent_ref01_data["id"],
    }
...
function policy_import_intent_basic_setup(extra)
  runner.load_env_local()

  local entity_data_file = _test_dir .. "../../.sdk/test/entity/policy_import_intent/PolicyImportIntentTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read policy_import_intent test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("KOTA_TEST_POLICY_IMPORT_INTENT_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["KOTA_TEST_POLICY_IMPORT_INTENT_ENTID"])
