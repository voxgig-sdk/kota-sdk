...
describe("ProviderDirect", function()
  it("should direct-list-provider", function()
    local setup = provider_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-provider", setup.live and "live" or "unit")
  ...
  it("should direct-load-provider", function()
    local setup = provider_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-provider", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function provider_direct_setup(mockres)
