...
describe("PlanDirect", function()
  it("should direct-list-plan", function()
    local setup = plan_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-plan", setup.live and "live" or "unit")
  ...
  it("should direct-load-plan", function()
    local setup = plan_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-plan", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function plan_direct_setup(mockres)
