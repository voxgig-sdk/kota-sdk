...
describe("AssociatedPersonDirect", function()
  it("should direct-list-associated_person", function()
    local setup = associated_person_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-associated_person", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"employee01"}) do
  ...
  it("should direct-load-associated_person", function()
    local setup = associated_person_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-associated_person", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function associated_person_direct_setup(mockres)
