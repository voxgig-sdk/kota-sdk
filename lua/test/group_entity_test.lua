...
describe("GroupEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Group(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Group(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set KOTA_TEST_GROUP_ENTID JSON to run live")
      return
    end
    ...
    -- LIST
    local group_ref01_match = {}

    local group_ref01_list_result, err = group_ref01_ent:list(group_ref01_match, nil)
    assert.is_nil(err)
    assert.is_table(group_ref01_list_result)
    ...
    local group_ref01_markdef_up0_name = "employer_id"
    ...
    assert.is_nil(err)
    local group_ref01_data_dt0_load_result = helpers.to_map(group_ref01_data_dt0_loaded)
...
function group_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("KOTA_TEST_GROUP_ENTID")
