...
describe("DependentsManagementIntentEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:DependentsManagementIntent(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = dependents_management_intent_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create", "load"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "dependents_management_intent." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set KOTA_TEST_DEPENDENTS_MANAGEMENT_INTENT_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local dependents_management_intent_ref01_ent = client:DependentsManagementIntent(nil)
    local dependents_management_intent_ref01_data = helpers.to_map(vs.getprop(
    ...
    -- LOAD
    local dependents_management_intent_ref01_match_dt0 = {
...
function dependents_management_intent_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/dependents_management_intent/DependentsManagementIntentTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read dependents_management_intent test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("KOTA_TEST_DEPENDENTS_MANAGEMENT_INTENT_ENTID")
