-- ContributionReport direct test
...
describe("ContributionReportDirect", function()
  it("should direct-list-contribution_report", function()
    local setup = contribution_report_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-contribution_report", setup.live and "live" or "unit")
  ...
  it("should direct-load-contribution_report", function()
    local setup = contribution_report_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-contribution_report", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function contribution_report_direct_setup(mockres)
