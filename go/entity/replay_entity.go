...
func NewReplayEntity(client *core.KotaSDK, entopts map[string]any) *ReplayEntity {
	if entopts == nil {
		entopts = map[string]any{}
	}
...
func (e *ReplayEntity) GetName() string { return e.name }
...
func (e *ReplayEntity) MarkDeleted() {
	e.deleted = true
}


// Deleted reports whether a successful Remove has resolved on this instance.
func (e *ReplayEntity) Deleted() bool {
	return e.deleted
}


func (e *ReplayEntity) Make() core.Entity {
	opts := map[string]any{}
	for k, v := range e.entopts {
		opts[k] = v
	}
	return NewReplayEntity(e.client, opts)
}

func (e *ReplayEntity) Data(args ...any) any {
...
func (e *ReplayEntity) Match(args ...any) any {
	if len(args) > 0 && args[0] != nil {
...
// argument it returns the current data as an Replay; with an argument it
// sets the data and returns the stored value. It delegates to the untyped Data
// (identical runtime) and converts at the typed boundary.
func (e *ReplayEntity) DataTyped(data ...Replay) Replay {
	if len(data) > 0 {
		return typedFrom[Replay](e.Data(asMap(data[0])))
	}
	return typedFrom[Replay](e.Data())
}

// MatchTyped mirrors DataTyped for the entity's match filter. The match is a
// partial of the entity, so it round-trips through Replay (all fields
// optional at the wire level).
func (e *ReplayEntity) MatchTyped(match ...Replay) Replay {
	if len(match) > 0 {
		return typedFrom[Replay](e.Match(asMap(match[0])))
	}
	return typedFrom[Replay](e.Match())
}
...
func (e *ReplayEntity) Stream(action string, args map[string]any, callopts map[string]any) <-chan any {
	out := make(chan any)
...
func (e *ReplayEntity) Load(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("load", e.name)
}


func (e *ReplayEntity) List(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("list", e.name)
}



func (e *ReplayEntity) Create(reqdata map[string]any, ctrl map[string]any) (any, error) {
	utility := e.utility
...
// ReplayCreateData and returns an Replay. It delegates to the untyped
// Create (identical runtime) and converts at the typed boundary.
func (e *ReplayEntity) CreateTyped(reqdata ReplayCreateData, ctrl map[string]any) (Replay, error) {
	res, err := e.Create(asMap(reqdata), ctrl)
	if err != nil {
		return Replay{}, err
	}
	return typedFrom[Replay](res), nil
}



func (e *ReplayEntity) Update(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("update", e.name)
}


func (e *ReplayEntity) Remove(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("remove", e.name)
}


func (e *ReplayEntity) runOp(ctx *core.Context, postDone func()) (any, error) {
	utility := e.utility
