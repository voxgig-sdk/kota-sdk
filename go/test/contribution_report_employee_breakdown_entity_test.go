...
func TestContributionReportEmployeeBreakdownEntity(t *testing.T) {
	t.Run("instance", func(t *testing.T) {
		testsdk := sdk.TestSDK(nil, nil)
		ent := testsdk.ContributionReportEmployeeBreakdown(nil)
		if ent == nil {
			t.Fatal("expected non-nil ContributionReportEmployeeBreakdownEntity")
		}
	})

	t.Run("basic", func(t *testing.T) {
		setup := contribution_report_employee_breakdownBasicSetup(nil)
		...
		for _, _op := range []string{"load"} {
			if _shouldSkip, _reason := isControlSkipped("entityOp", "contribution_report_employee_breakdown." + _op, _mode); _shouldSkip {
				if _reason == "" {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set KOTA_TEST_CONTRIBUTION_REPORT_EMPLOYEE_BREAKDOWN_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		_ = contributionReportEmployeeBreakdownRef01Data
...
func contribution_report_employee_breakdownBasicSetup(extra map[string]any) *entityTestSetup {
	loadEnvLocal()

	_, filename, _, _ := runtime.Caller(0)
	dir := filepath.Dir(filename)

	entityDataFile := filepath.Join(dir, "..", "..", ".sdk", "test", "entity", "contribution_report_employee_breakdown", "ContributionReportEmployeeBreakdownTestData.json")

	entityDataSource, err := os.ReadFile(entityDataFile)
	if err != nil {
		panic("failed to read contribution_report_employee_breakdown test data: " + err.Error())
	}

	var entityData map[string]any
	if err := json.Unmarshal(entityDataSource, &entityData); err != nil {
		panic("failed to parse contribution_report_employee_breakdown test data: " + err.Error())
	}
	...
	entidEnvRaw := os.Getenv("KOTA_TEST_CONTRIBUTION_REPORT_EMPLOYEE_BREAKDOWN_ENTID")
