...
func TestGroupEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set KOTA_TEST_GROUP_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		groupRef01MarkdefUp0Name := "employer_id"
		...
		groupRef01DataDt0LoadResult := core.ToMapAny(groupRef01DataDt0Loaded)
...
func groupBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("KOTA_TEST_GROUP_ENTID")
	...
	idmapResolved := core.ToMapAny(env["KOTA_TEST_GROUP_ENTID"])
