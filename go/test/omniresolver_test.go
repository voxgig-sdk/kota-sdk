...
import (
	...
	omni "github.com/voxgig-sdk/kota-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.KotaError); is {
				out := map[string]any{
					"name":    "KotaError",
...
func omniCtx(arg any, client *sdk.KotaSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.KotaSDK); is {
			client = live
