...
func TestProviderEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Provider(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Provider(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set KOTA_TEST_PROVIDER_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		providerRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.provider", setup.data)))
		var providerRef01Data map[string]any
		if len(providerRef01DataRaw) > 0 {
			providerRef01Data = core.ToMapAny(providerRef01DataRaw[0][1])
		}
		...
		providerRef01Match := map[string]any{}

		providerRef01ListResult, err := providerRef01Ent.List(providerRef01Match, nil)
		if err != nil {
			t.Fatalf("list failed: %v", err)
		}
		_, providerRef01ListOk := providerRef01ListResult.([]any)
		if !providerRef01ListOk {
			t.Fatalf("expected list result to be an array, got %T", providerRef01ListResult)
		}
		...
		providerRef01DataDt0LoadResult := core.ToMapAny(providerRef01DataDt0Loaded)
...
func providerBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap := vs.Transform(
		[]any{"provider01", "provider02", "provider03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("KOTA_TEST_PROVIDER_ENTID")
	...
	idmapResolved := core.ToMapAny(env["KOTA_TEST_PROVIDER_ENTID"])
