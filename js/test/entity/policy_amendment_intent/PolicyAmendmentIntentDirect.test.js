...
describe('PolicyAmendmentIntentDirect', async () => {
  ...
  test('direct-load-policy_amendment_intent', async (t) => {
    ...
    if (setup.live) {
      const listResult = await client.direct({
        ...
        method: 'GET',
        params: {
        id: setup.idmap['policy_amendment_intent01'],
        },
      })
      ...
      params.policy_id = setup.idmap['policy01']
    } else {
      params.id = 'direct01'
  ...
  test('direct-list-policy_amendment_intent', async (t) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
