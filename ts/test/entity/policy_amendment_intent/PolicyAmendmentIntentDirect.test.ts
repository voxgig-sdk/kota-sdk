...
describe('PolicyAmendmentIntentDirect', async () => {
  ...
  test('direct-load-policy_amendment_intent', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-policy_amendment_intent', setup.live)) return
  ...
  test('direct-list-policy_amendment_intent', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-policy_amendment_intent', setup.live)) return
    if (skipIfMissingIds(t, setup, ["policy_amendment_intent01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['KOTA_TEST_POLICY_AMENDMENT_INTENT_ENTID']
