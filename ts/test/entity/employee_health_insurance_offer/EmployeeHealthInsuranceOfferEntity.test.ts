...
describe('EmployeeHealthInsuranceOfferEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'employee_health_insurance_offer.' + op, live)) return
    }
    ...
    let employee_health_insurance_offer_ref01_data = Object.values(setup.data.existing.employee_health_insurance_offer)[0] as any

    // LOAD
    const employee_health_insurance_offer_ref01_ent = client.EmployeeHealthInsuranceOffer()
    const employee_health_insurance_offer_ref01_match_dt0: any = {}
    employee_health_insurance_offer_ref01_match_dt0.id = employee_health_insurance_offer_ref01_data.id
    const employee_health_insurance_offer_ref01_data_dt0 = await employee_health_insurance_offer_ref01_ent.load(employee_health_insurance_offer_ref01_match_dt0)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['KOTA_TEST_EMPLOYEE_HEALTH_INSURANCE_OFFER_ENTID']
