...
describe('EmployerHealthInsurancePolicyResponsePagedListEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'employer_health_insurance_policy_response_paged_list.' + op, live)) return
    }
    ...
    let employer_health_insurance_policy_response_paged_list_ref01_data = Object.values(setup.data.existing.employer_health_insurance_policy_response_paged_list)[0] as any
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['KOTA_TEST_EMPLOYER_HEALTH_INSURANCE_POLICY_RESPONSE_PAGED_LIST_ENTID']
