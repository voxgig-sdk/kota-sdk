...
describe('EmployeeHealthInsurancePolicyEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'employee_health_insurance_policy.' + op, live)) return
    }
    ...
    let employee_health_insurance_policy_ref01_data = Object.values(setup.data.existing.employee_health_insurance_policy)[0] as any

    // LOAD
    const employee_health_insurance_policy_ref01_ent = client.EmployeeHealthInsurancePolicy()
    const employee_health_insurance_policy_ref01_match_dt0: any = {}
    ...
    assert(employee_health_insurance_policy_ref01_data_dt0.id === employee_health_insurance_policy_ref01_data.id)
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/employee_health_insurance_policy/EmployeeHealthInsurancePolicyTestData.json')
  ...
  const idmapEnvVal = process.env['KOTA_TEST_EMPLOYEE_HEALTH_INSURANCE_POLICY_ENTID']
