...
describe('EmployeeHealthInsuranceOfferResponsePagedListEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'employee_health_insurance_offer_response_paged_list.' + op, live)) return
    }
    ...
    let employee_health_insurance_offer_response_paged_list_ref01_data = Object.values(setup.data.existing.employee_health_insurance_offer_response_paged_list)[0] as any

    // LIST
    const employee_health_insurance_offer_response_paged_list_ref01_ent = client.EmployeeHealthInsuranceOfferResponsePagedList()
    const employee_health_insurance_offer_response_paged_list_ref01_match: any = {}
    employee_health_insurance_offer_response_paged_list_ref01_match['employee_id'] = setup.idmap['employee01']

    const employee_health_insurance_offer_response_paged_list_ref01_list = (await employee_health_insurance_offer_response_paged_list_ref01_ent.list(employee_health_insurance_offer_response_paged_list_ref01_match)).map((e: any) => e.data())
