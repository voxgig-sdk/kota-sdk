...
describe('AssociatedPersonEligibilityResponsePagedListDirect', async () => {
  ...
  test('direct-list-associated_person_eligibility_response_paged_list', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-associated_person_eligibility_response_paged_list', setup.live)) return
    if (skipIfMissingIds(t, setup, ["dependents_management_intent01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['KOTA_TEST_ASSOCIATED_PERSON_ELIGIBILITY_RESPONSE_PAGED_LIST_ENTID']
