...
describe('AssociatedPersonEligibilityResponsePagedListEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'associated_person_eligibility_response_paged_list.' + op, live)) return
    }
    ...
    let associated_person_eligibility_response_paged_list_ref01_data = Object.values(setup.data.existing.associated_person_eligibility_response_paged_list)[0] as any
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['KOTA_TEST_ASSOCIATED_PERSON_ELIGIBILITY_RESPONSE_PAGED_LIST_ENTID']
