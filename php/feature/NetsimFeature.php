...
class KotaNetsimFeature extends KotaBaseFeature
{
    ...
    private function _simulate(KotaContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(KotaContext $ctx, array $applied): void
    {
        $client = $this->client;
