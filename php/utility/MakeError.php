...
class KotaMakeError
{
    public static function call(?KotaContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new KotaContext([], null);
        }
        $op = $ctx->op ?? new KotaOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new KotaResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof KotaError) ? $err->msg : (string)$err;
        $msg = "KotaSDK: {$opname}: {$errmsg}";
