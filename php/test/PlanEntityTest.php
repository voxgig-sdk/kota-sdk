...
class PlanEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Plan(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Plan(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = plan_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "plan." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set KOTA_TEST_PLAN_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $plan_ref01_data = null;
        if (count($plan_ref01_data_raw) > 0) {
            $plan_ref01_data = Helpers::to_map($plan_ref01_data_raw[0][1]);
        }

        // LIST
        $plan_ref01_ent = $client->Plan(null);
        $plan_ref01_match = [];

        $plan_ref01_list_result = $plan_ref01_ent->list($plan_ref01_match, null);
        $this->assertIsArray($plan_ref01_list_result);
...
function plan_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/plan/PlanTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["plan01", "plan02", "plan03"] as $k) {
    ...
    $entid_env_raw = getenv("KOTA_TEST_PLAN_ENTID");
