...
class GroupQuoteIntentEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = KotaSDK::test(null, null);
        $ent = $testsdk->GroupQuoteIntent(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->GroupQuoteIntent(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->GroupQuoteIntent(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = group_quote_intent_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create", "list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "group_quote_intent." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set KOTA_TEST_GROUP_QUOTE_INTENT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $group_quote_intent_ref01_ent = $client->GroupQuoteIntent(null);
        ...
        $group_quote_intent_ref01_list_result = $group_quote_intent_ref01_ent->list($group_quote_intent_ref01_match, null);
        $this->assertIsArray($group_quote_intent_ref01_list_result);
...
function group_quote_intent_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/group_quote_intent/GroupQuoteIntentTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["group_quote_intent01", "group_quote_intent02", "group_quote_intent03"] as $k) {
    ...
    $entid_env_raw = getenv("KOTA_TEST_GROUP_QUOTE_INTENT_ENTID");
