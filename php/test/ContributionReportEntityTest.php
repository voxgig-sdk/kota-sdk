...
class ContributionReportEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = KotaSDK::test(null, null);
        $ent = $testsdk->ContributionReport(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->ContributionReport(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->ContributionReport(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = contribution_report_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create", "list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "contribution_report." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set KOTA_TEST_CONTRIBUTION_REPORT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $contribution_report_ref01_ent = $client->ContributionReport(null);
        ...
        $contribution_report_ref01_list_result = $contribution_report_ref01_ent->list($contribution_report_ref01_match, null);
        $this->assertIsArray($contribution_report_ref01_list_result);
...
function contribution_report_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/contribution_report/ContributionReportTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["contribution_report01", "contribution_report02", "contribution_report03"] as $k) {
    ...
    $entid_env_raw = getenv("KOTA_TEST_CONTRIBUTION_REPORT_ENTID");
