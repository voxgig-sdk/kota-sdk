...
class FtCtrl extends KotaControl
{
...
class FtClient
{
    ...
    private KotaUtility $utility;

    public function __construct(KotaUtility $utility, array $options)
    {
    ...
    public function get_utility(): KotaUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (KotaContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (KotaContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = KotaConfig::make_feature($name);
    ...
    private static function buildUrl(KotaSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof KotaError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new KotaResponse($fetched);
        }
    ...
    private function fail(KotaContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (KotaContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
