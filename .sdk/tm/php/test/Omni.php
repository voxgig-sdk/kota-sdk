...
//    only - a typed KotaContext there would make every ctx
...
final class KotaOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new KotaContext($clean, null);
    ...
    public static function err_from_map($m): ?KotaError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new KotaError(is_string($code) ? $code : '', $msg);
    }
